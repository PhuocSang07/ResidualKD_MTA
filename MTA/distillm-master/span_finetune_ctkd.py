...
def prepare_span_indices_and_weights(t_layer_weights, s_layer_weights, 
    ...
    elif offsets_mapping.shape[1] < SeqLen:
        pad_size = SeqLen - offsets_mapping.shape[1]
        padding = torch.zeros(B_size, pad_size, 2, dtype=offsets_mapping.dtype, device=device)
        current_offsets_mapping = torch.cat([offsets_mapping.to(device), padding], dim=1)
    else:
...
def main():
    ...
    if teacher_model is not None:
        student_hidden_size = getattr(model.config, 'hidden_size', None) or getattr(model.config, 'n_embd', None)
        teacher_hidden_size = getattr(teacher_model.config, 'hidden_size', None) or getattr(teacher_model.config, 'n_embd', None)
