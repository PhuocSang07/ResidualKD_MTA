...
# Base directory of this file — works in any environment regardless of $HOME
...
def create_prompt(task: str, few_shot: int, **args):
    ...
    if args.get('sys_user', False):
        prompt += json.load(open(_PROMPT_DIR / "context.json"))[task] + "\n\n"

    module = __load_module_from_py_file(str(_PROMPT_DIR / "few_shot" / f"{task}.py"))
...
def create_chat_prompt(task: str, few_shot: int, **args):
    chat, sys_prompt = [], json.load(open(_PROMPT_DIR / "context.json"))[task]
    module = __load_module_from_py_file(str(_PROMPT_DIR / "few_shot" / f"{task}.py"))
